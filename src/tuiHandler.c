...
const Color BG_2_COLOR = (Color){10, 26, 51};
const Color BG_3_COLOR = (Color){32, 47, 71};
const Color BG_4_COLOR = (Color){36, 52, 79};
const Color BG_COLOR_HIGHLIGHT = (Color){42, 59, 89};
const Color BG_COLOR_HIGHLIGHT2 = (Color){56, 83, 130};
...
const Color FONT_COLOR_HIGHLIGHT = (Color){200, 200, 200};
...
void initUI() {
  ...
  addStringToTable("Este es un child element y su contenido es muy largo y "
                   "sobre sale del espacio",
                   &(childTreeView->texts));
  ...
  for (int i = 0; i < 30; i++) {
    treeViewElem = newTreeViewElement(treeViewElem, 0);
    addStringToTable("Test Parent element", &(treeViewElem->texts));
    treeViewElem->collapsed = 1;
    childTreeView = newTreeViewElement(treeViewElem, 1);
    addStringToTable("Test element", &(childTreeView->texts));
    childTreeView2 = newTreeViewElement(childTreeView, 1);
    addStringToTable("Este es nieto", &(childTreeView2->texts));
    childTreeView = newTreeViewElement(childTreeView, 0);
    addStringToTable("Este es otro hijo", &(childTreeView->texts));
    childTreeView = newTreeViewElement(childTreeView, 0);
...
void updateComponent(Component *component, int resize) {
  ...
  addComponentToTable(component, &cmpsToUpdate);
}
...
void markComponentUpdated(Component *component) {
  ...
  case tabview_t:
    if (component->tabview_properties.selectedTab < component->childCount &&
        component->children[component->tabview_properties.selectedTab] != NULL)
      markComponentUpdated(
          component->children[component->tabview_properties.selectedTab]);
    break;
  default:
    for (int i = 0; i < component->childCount; i++) {
      markComponentUpdated(component->children[i]);
    }
    break;
  }
}
...
void markComponentResized(Component *component) {
  ...
  for (int i = 0; i < component->childCount; i++) {
    markComponentResized(component->children[i]);
  }
}
...
void closeProgram() { restoreTerminal(); }
...
void drawViewport(Component *component, Component *parent) {
  ...
  if (component->isUpdated != 0) {
    ...
    debug("Window Size: %d cols %d rows %dx %dy", ws.ws_col, ws.ws_row,
          ws.ws_xpixel, ws.ws_ypixel);
    debug("Component Size: %d cols %d rows", component->real_width,
          component->real_height);
    component->viewport_properties.vp_settings->y =
        component->y + parent->global_y;
    component->viewport_properties.vp_settings->x =
        component->x + parent->global_x;
    component->viewport_properties.vp_settings->screen_width =
        component->real_width * ws.ws_xpixel / ws.ws_col;
    component->viewport_properties.vp_settings->screen_height =
        component->real_height * ws.ws_ypixel / ws.ws_row;
    component->viewport_properties.vp_settings->render_settings->screen_height =
        component->viewport_properties.vp_settings->screen_height;
...
void drawTabView(Component *component, Component *parent) {
  ...
  if (component->isUpdated != 0) {
    ...
    int max_tab_sapce =
        component->real_width - arrowLengthEnd - arrowLengthStart;
    ...
    if (component->tabview_properties.snapTab) {
      for (int i = 0; i < component->tabview_properties.tabCount; i++) {
        if (i != component->tabview_properties.selectedTab) {
          ...
          int len_tab =
              strlen(component->tabview_properties.tabTitles.table[i]);
    ...
    for (int i = 0; i < component->tabview_properties.tabCount; i++) {
      ...
      int max_titleLength =
          start_x + component->real_width - aux_x - arrowLengthEnd;
      int titleLength =
          strlen(component->tabview_properties.tabTitles.table[i]);
      ...
      if (titleLength > 0) {
        strncpy(title,
                component->tabview_properties.tabTitles.table[i] + auxOffset,
                titleLength);
        title[titleLength] = '\0';
...
void drawTextComponent(Component *component) {
  ...
  if (component->isUpdated != 0) {
    ...
    debug("Printing text %s in text of size %dx %dy. Len: %d",
          ...
          component->real_height, strlen(component->text_properties.content));
    ...
    for (int i = 0; i < finalStringLength + 1; i++) {
      ...
      if (finalString[i] == '\n' || finalString[i] == '\r' ||
          finalString[i] == '\0' || i == strlen(finalString)) {
        finalString[i] = '\0';
        debug("Printing final String: %s\n At: %dx, %dy",
              finalString + init_line, component->global_x,
...
void drawTreeView(Component *component) {
  ...
  debug("Parent local coords: %dx %dy", component->parent->x,
        component->parent->y);
  debug("Parent global coords: %dx %dy", component->parent->global_x,
        component->parent->global_y);
  ...
  int highlightContext = 0;
  int auxOffset = 0;
  int isScrolled = 0;

  // Check the offset
  if (component->treeview_properties.snapElement) {
    while (current != NULL) {
      if (auxOffset >= component->treeview_properties.offset) {
        actualHeight++;
      }
      if (current->status == 2) {
        // Calculate correct offset
        // If object is in upper bound
        if (actualHeight == -1) {
          component->treeview_properties.offset = auxOffset;
          isScrolled = 1;
        } else if (actualHeight >= component->real_height) {
          component->treeview_properties.offset =
              auxOffset - component->real_height + 1;
          isScrolled = 1;
        }
        break;
      }
  ...
  actualHeight = 0;
  auxOffset = 0;
  current = component->treeview_properties.child;
  while (current != NULL) {
    if (auxOffset >= component->treeview_properties.offset) {
      //if (isScrolled || current->isUpdated) {
        //current->isUpdated = 0;
        ...
        if (current->childElement != NULL) {
          ...
          sprintf(textElement->text_properties.content, "[%s] %s",
                  current->texts.table[current->collapsed],
        ...
        case 0:
          ...
          textElement->text_properties.bgColor =
              component->treeview_properties
                  .colors[4 + alternateCounter * alternattingPattern];
          textElement->text_properties.textColor =
              component->treeview_properties
                  .colors[5 + alternateCounter * alternattingPattern];
          break;
        case 1:
          // Hover status
          textElement->text_properties.bgColor =
              component->treeview_properties.colors[2];
          textElement->text_properties.textColor =
              component->treeview_properties.colors[3];
          break;
        case 2:
          // Selected/Focus
          textElement->text_properties.bgColor =
              component->treeview_properties.colors[0];
          textElement->text_properties.textColor =
        ...
        for (int i = 0; i < hierarchyLevel; i++) {
          if (i < hierarchyLevel - 1)
            stringToDraw = " │ ";
          else if (current->nextElement == NULL)
            stringToDraw = " └ ";
          else
            stringToDraw = " ├ ";
          // Draw highlighted context only if we are at the correct level
          printText(
              component->global_x + i * 3 + 1,
              component->global_y + actualHeight + 1, stringToDraw,
    ...
    if (!current->collapsed && current->childElement != NULL) {
      ...
      hierarchyLevel++;
      if (highlightContext > 0)
        highlightContext++;
    } else if (current->nextElement != NULL)
      current = current->nextElement;
    else if (current->parentElement != NULL &&
             current->parentElement->nextElement != NULL) {
    ...
    auxOffset++;
    if (actualHeight >= component->real_height)
      break;

    alternateCounter = alternateCounter == (alternattingPattern - 1)
                           ? 0
                           : alternateCounter + 1;

    // Hightlighting context
    if (highlightContext == 0) {
      if (component->treeview_properties.highlightMode == 0 &&
...
/*Function that pre-process a component, updating properties for further
 * calculation*/
void preCalculateComponent(Component *component, Component *parent) {
...
void calculateComponentDimensions(Component *component, Component *parent) {
  ...
  if (component->real_width == -1)
    calculateComponentDimensionsWidth(component, parent);
  if (component->real_height == -1)
...
/*Function that calculates and populates the dimensions and positions of a
...
void calculateComponentDimensionsHeight(Component *component,
  ...
  if (parent->real_height == 0 && (component->topToBottomOf == parent ||
                                   component->bottomToBottomOf == parent))
    return;
  // Calculates the dimensions
  int componentMarginTop =
      component->marginTop == -1 ? component->margin : component->marginTop;
  int componentMarginBot =
      component->marginBot == -1 ? component->margin : component->marginBot;
  int componentPaddingTop =
      parent->paddingTop == -1 ? parent->padding : parent->paddingTop;
  int componentPaddingBot =
      parent->paddingBot == -1 ? parent->padding : parent->paddingBot;
  int min_y = -1;
  int max_y = -1;
  ...
  if (component->topToTopOf != NULL && component->topToTopOf != parent) {
    ...
    if (component->topToBottomOf == parent) {
      ...
      min_y =
          component->topToBottomOf->y + component->topToBottomOf->real_height;
    }
    min_y += componentMarginTop;
  } else if ((component->bottomToBottomOf == NULL &&
              component->bottomToTopOf == NULL) ||
             component->topToTopOf == parent) {
    // Default: topToTopOf: Parent
    int parentPadding =
        parent->paddingTop != -1 ? parent->paddingTop : parent->padding;
  ...
  if (component->bottomToTopOf != NULL) {
    ...
    if (component->bottomToBottomOf == parent) {
      ...
      auxHeight = component->bottomToBottomOf->y +
                  component->bottomToBottomOf->real_height;
    }
  ...
  if (component->autoHeight == 0 || component->autoHeight == 1) {
    ...
    if (component->maxHeight != -1) {
      if (component->real_height > component->maxHeight)
        component->real_height = component->maxHeight;
    }
    if (component->minHeight != -1) {
      if (component->real_height < component->minHeight)
        component->real_height = component->minHeight;
    }
    ...
    if (max_y == -1 || min_y == -1)
      ...
      component->real_height = max_y - min_y;

    // Adjust size according to bias

    component->real_height *=
        component->heightBias != -1 ? component->heightBias : 1;
...
void calculateComponentDimensionsWidth(Component *component,
  ...
  if (parent->real_width == 0 &&
      (component->startToEndOf == parent || component->endToEndOf == parent))
    return;
  // Calculates the dimensions
  int componentMarginStart =
      component->marginStart == -1 ? component->margin : component->marginStart;
  int componentMarginEnd =
      component->marginEnd == -1 ? component->margin : component->marginEnd;
  int componentPaddingStart =
      parent->paddingStart == -1 ? parent->padding : parent->paddingStart;
  int componentPaddingEnd =
      parent->paddingEnd == -1 ? parent->padding : parent->paddingEnd;
  int min_x = -1;
  int max_x = -1;
  ...
  if (component->startToStartOf != NULL &&
      component->startToStartOf != parent) {
    ...
    // Default: topToTopOf: Parent
    int parentPadding =
        parent->paddingStart != -1 ? parent->paddingStart : parent->padding;
...
Component *newTabView() {
  ...
  cont->tabview_properties.bgColor = BG_3_COLOR;
  cont->tabview_properties.bgFocusColor = BG_3_COLOR;
  cont->tabview_properties.bgSelectedColor =BG_COLOR_HIGHLIGHT2;
  cont->tabview_properties.txtColor = FONT_2_COLOR;
  cont->tabview_properties.txtFocusColor = FONT_2_COLOR;
  cont->tabview_properties.txtSelectedColor = FONT_COLOR_HIGHLIGHT;
...
Component *newTreeViewComponent() {
  ...
  cmp->treeview_properties.colors[0] = BG_COLOR_HIGHLIGHT;
  cmp->treeview_properties.colors[1] = FONT_COLOR_HIGHLIGHT;
  cmp->treeview_properties.colors[2] = BG_2_COLOR;
  cmp->treeview_properties.colors[3] = FONT_2_COLOR;
  cmp->treeview_properties.colors[4] = BG_COLOR;
  cmp->treeview_properties.colors[5] = FONT_2_COLOR;
  cmp->treeview_properties.colors[6] = BG_2_COLOR;
  cmp->treeview_properties.colors[7] = FONT_2_COLOR;
  cmp->treeview_properties.highlightMode = 0;
...
TreeViewElement *newTreeViewElement(TreeViewElement *parent, int isChild) {
  ...
  if (isChild == 0 && parent != NULL)
    parent->nextElement = elem;
  else if (isChild == 1 && parent != NULL)
...
/*Function that creates a new table of strings*/
ComponentTable newComponentTable(Component *cmp) {
  ComponentTable retTable;
  retTable.table = malloc(sizeof(Component *));
  retTable.table[0] = cmp;
  retTable.length = 1;
  return retTable;
}
/*******************************************************************/
...
/*Function that return previous treeViewElement in order, not caring about
 * height*/
/*careAbougCollapsed: 0 assume not collapsed, 1 care about it, 2 assume
 * collapsed*/
TreeViewElement *getPrevTreeViewElement(TreeViewElement *element,
                                        int careAboutCollapsed) {
  ...
  if (element->prevElement != NULL) {
    ...
    while (current != NULL) {
      prev = current;
      current = careAboutCollapsed
                    ? getNextTreeViewElement(prev, current->collapsed)
                    : getNextTreeViewElement(prev, 0);
      if (current == element)
        break;
    }
...
void addComponentToTable(Component *component, ComponentTable *table) {
  debug("Adding element to component table of size %d. cmp: %d", table->length,
        component);
  Component **auxTable = malloc(sizeof(Component *) * (table->length + 1));
  memcpy(auxTable, table->table, table->length * sizeof(Component *));
  if (table->length > 0)
    free(table->table);
  table->table = auxTable;
  table->table[table->length] = component;
  table->length++;
}

/*Function that empties a component table*/
void emptyComponentTable(ComponentTable *table) {
  debug("Emptying component table of length %d", table->length);
  if (table->length > 0) {
